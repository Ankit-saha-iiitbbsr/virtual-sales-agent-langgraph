...
class DatabaseManager:
    ...
    timestamp: str,
        ...
        """
        query = """
        INSERT INTO chat_history (message_id, customer_id, message, response, timestamp)
        VALUES (?, ?, ?, ?, ?);
        """
        try:
            with self.get_connection() as conn:
                conn.execute(
                    query,
                    (message_id, customer_id, message, response, timestamp)
                )
                logger.info(f"Successfully saved chat message: {message_id}")
                conn.commit()
            return True
        
        except sqlite3.Error as e:
            logger.error(f"Error saving chat message {message_id}: {e}")
            return False

    def get_customer_chat_history(self, customer_id: int) -> list:
        """
        ...
        Returns:
            list: List of chat messages and responses
        """
        query = """
            SELECT message_id, customer_id, message, response, timestamp
            FROM chat_history
            WHERE customer_id = ?
        ...
        try:
            ...
            logger.error(f"Error retrieving chat history for customer {customer_id}: {e}")
            return []
    ...
    created_at: str
        ...
        query = """
        INSERT INTO pending_orders (customer_id, order_details, created_at, status)
        VALUES (?, ?, ?, 'AWAITING_CONFIRMATION');
        """
        try:
            with self.get_connection() as conn:
                conn.execute(query, (customer_id, order_details, created_at))
                conn.commit()
            return True
        except sqlite3.Error as e:
    ...
    def get_pending_order(self, customer_id: int) -> Optional[dict]:
        """Gets the most recent pending order for a customer."""
        query = """
    ...
    def confirm_pending_order(self, customer_id: int) -> Optional[dict]:
        """Confirms a pending order."""
        try:
            with self.get_connection() as conn:
                # Get the pending order
                pending_order = self.get_pending_order(customer_id)
